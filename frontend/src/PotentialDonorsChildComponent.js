...
const PotentialDonorsChildComponent = ({ potentialDonors, recipientData }) => {
    ...
    const requestDonorNotify = () => {
        ...
        if (potentialDonors && potentialDonors.length > 0) {
            ...
            axios.post("http://ec2-15-207-165-21.ap-south-1.compute.amazonaws.com:4000/api/donations", { recipient_email: recipientData.email })
                .then((response) => {
                    console.log(response.data);
                    const donation_id = response.data.donation_id;

                    const requests = potentialDonors.map((donor) =>
                        axios.post("http://ec2-15-207-165-21.ap-south-1.compute.amazonaws.com:4000/api/requested-donors", { donation_id, donor_email: donor.email })
                    );

                    Promise.all(requests)
                        .then((responses) => {
                            axios.post("http://ec2-15-207-165-21.ap-south-1.compute.amazonaws.com:4000/api/send-mail", { donation_id, potentialDonors, recipientData })
                                .then((response) => {
                                    console.log(response.data);
