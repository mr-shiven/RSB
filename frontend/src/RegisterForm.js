...
export default function RegisterForm() {
    ...
    const handleSubmit = (event) => {
        ...
        if (userRole === 'Donor') {
            axios.post('http://localhost:4000/api/donors', data)
                .then(response => {
            ...
            axios.post('http://localhost:4000/api/recipients', data)
                .then(response => {
                    console.log(response);
    ...
    async function validateOtp(otp) {
        try {
            const response = await axios.post('http://localhost:4000/validate-otp', { otp });
